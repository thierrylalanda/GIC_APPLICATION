...
$(document).ready(function () {
    ...
    function getContent(data) {
        ...
        content.push({text: 'Consommation des ' + categorie + ' ' + title, style: 'title'});
        content.push({text: 'periode ' + parseDate(periode), style: 'subtitle'});
        for (var j = 0; j < 4; j++) {
            this.table = getBody();
            content.push(this.table.legend);
            content.push({
                style: 'tableExample',
                table: {
    ...
    function getBody() {
        var body = new Array();
        var objet = [{Date: "thierry", personnel: "henri", Article: "thth", Qté: 22, PrixTotal: 243656, PU: 123},
            {Date: "lalanda", personnel: "lalanda", Article: "thth", Qté: 22, PrixTotal: 243656, PU: 123},
            {Date: "thierry", personnel: "MKT", Article: "thth", Qté: 22, PrixTotal: 243656, PU: 123}];
        var _header = new Array();
        var cles = getKeys(objet);
        ...
        for (var i = 0; i < objet.length; i++) {
            ...
            var total = 0;
            total += objet[i].PrixTotal;
            for (var j = 0; j < cles.length; j++) {


                _content.push({text: objet[i][cles[j]], style: 'tableContent'});
            }
        ...
        return {body: body, widht: getwidth(objet), legend: getLegend(), total: total};
    }

    function getLegend() {

        var legend = {text: "service : Helpdesk", fontSize: 10, bold: true, margin: [0, 15, 0, 3]};
        return legend;
    }
    ...
    function printPdf(data) {
        var docDefinition = {
            ...
            footer: function (currentPage, pageCount) {
                ...
                return tfoo;
                //[{text: currentPage.toString() + ' / ' + pageCount, alignment: 'right', style: 'footer'}, {text: "imprimé le " + getDate(), alignment: 'left', style: 'footerleft'}, {text: "GIC ", alignment: 'center', style: 'footercenter'}];
            },
    ...
    function getMulti(serveur, param, idResponse) {
    $.post(
            serveur,
            param,
            function (data) {
    ...
    $(".region_service").click(function (e) {
        ...
        if($(this).hasClass("for_end")){
            getMulti("admin", param, ".site_service");
        }else{
